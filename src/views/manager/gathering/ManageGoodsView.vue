<template>
    ...
    <template v-if="control === 'send'">
        <el-table :data="dataTableSend" v-loading="loadingTable" :default-sort="{ prop: 'stt', order: 'ascending' }">
            ...
            <el-table-column prop="weight" label="Cân nặng"></el-table-column>
            <el-table-column prop="creator" label="Người tạo"></el-table-column>
            ...
            <el-table-column prop="shippingFee" label="Phí vận chuyển" sortable></el-table-column>
...
const getPackageToSend = async (page: any) => {
    ...
    try {
        dataTableSend.value = [];
        const res = await PackageServices.getPackageFromGatheringInGatheringPoint(authStore.userInfo, page, httpJwt);
...
const getReceivedPackage = async (page: any) => {
    ...
    try {
        ...
        const res = await PackageServices.getPackageToGatheringInGatheringPoint(authStore.userInfo, page, httpJwt);
